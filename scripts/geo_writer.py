...
- 风格可以随机，但是必须没有AI生成的味道，要像人写的一样。可以适当使用比喻、类比、故事等修辞手法来增强可读性和吸引力，但不要过度。

写作要求：
1. 语言：简体中文。
2. 结构：一个 H1 标题 + 引言 + 3-5 个小节 + 结语。开头要有基于消息渠道（如麦肯锡）+ 原文标题 研究
...
11.严格检查合规，避免文章出现中国和美国的敏感词汇和违规内容，不许发生侵权。必须将引用的原文标注数据来源，且数据来源必须是公开可查的、合法合规的渠道，不得使用任何未经授权的内部资料或敏感信息。文章内容必须符合中国法律法规和平台政策要求，避免涉及政治、宗教、暴力、色情等敏感话题，不得出现任何可能引发争议或违规的表述。
